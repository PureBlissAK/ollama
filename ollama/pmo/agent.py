...
# Lazy/optional runtime imports for heavy SDKs (GitHub, GCP).
# Use `importlib.util.find_spec` to avoid importing at module import time.
GITHUB_AVAILABLE = False
GCP_AVAILABLE = False
try:
    if importlib.util.find_spec("github"):
        # Import only the Github client symbol at runtime.
...
try:
    if importlib.util.find_spec("google.cloud"):
        # Import only the Cloud Build client symbol at runtime.
        from google.cloud import build_v1  # type: ignore
...
class PMOAgent:
    ...
    def __init__(
        ...
        self.github_repo: Any | None = None
        if GITHUB_AVAILABLE:
        ...
        if GCP_AVAILABLE and self.gcp_project:
            try:
                ...
                self.gcp_client = self.gcp_build_client
            except Exception as e:
    ...
    def load_pmo_yaml(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise PMOValidationError(f"Invalid YAML in pmo.yaml: {e}") from e
    ...
    def validate_pmo_yaml(self, data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if valid:
            ...
            logger.error(
                f"pmo.yaml validation failed: {len(errors)} errors, {len(warnings)} warnings"
            )
    ...
    def validate_compliance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            details["pmo_yaml"] = (
                f"{validation['score']}% ({validation['populated_labels']}/{validation['total_labels']})"
            )
        ...
        if workflows_dir.exists():
            ...
            details["workflows"] = (
                f"{len(required_workflows) - len(missing)}/{len(required_workflows)} workflows"
            )
        else:
        ...
        if hooks_dir.exists():
            existing_hooks = [
                f.name for f in hooks_dir.iterdir() if f.is_file() and os.access(f, os.X_OK)
            ]
            missing = [h for h in required_hooks if h not in existing_hooks]
            checks["git_hooks"] = len(missing) == 0
            details["git_hooks"] = (
                f"{len(required_hooks) - len(missing)}/{len(required_hooks)} hooks installed"
            )
        else:
    ...
    def setup_github_labels(self, force: bool = False) -> Dict[str, Any]:
        ...
        for _category, labels in self.GITHUB_LABELS.items():
    ...
    def auto_remediate_drift(self) -> Dict[str, Any]:
        ...
        if self.github_repo:
            try:
                labels_result = self.setup_github_labels(force=False)
                fixes["github_labels"] = labels_result["created"] > 0
                details["github_labels"] = f"Created {labels_result['created']} labels"
            except Exception as e:
        ...
        try:
            ...
            fixes["gpg_signing"] = proc.returncode == 0
                ...
                if pmo_data:
                    ...
                    if validation["score"] < 100:
                        ...
                        if updated:
                            ...
                            details["pmo_yaml_completion"] = (
                                f"Added missing labels, score now {self.validate_pmo_yaml(pmo_data)['score']}%"
                            )
                        else:
        ...
        if self.github_repo:
            try:
                ...
                if workflows_dir.exists():
                    ...
                    details["workflows_creation"] = (
                        f"Found {len(workflows_created)} missing workflows"
                        if workflows_created
                        else "All workflows exist"
                    )
                else:
    ...
    def create_build_trigger(
        ...
        try:
            ...
            raise PMOValidationError(f"Failed to create build trigger: {e}") from e

    def update_build_trigger(self, trigger_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise PMOValidationError(f"Failed to update build trigger: {e}") from e
    ...
    def list_build_triggers(self) -> Dict[str, Any]:
        ...
        try:
            ...
            request = cloudbuild_v1.ListBuildTriggersRequest(project_id=self.gcp_project)
                ...
                triggers.append(
                    {
                        ...
                        "branch_pattern": (trigger.github.push.branch if trigger.github else None),
            ...
            raise PMOValidationError(f"Failed to list build triggers: {e}") from e
