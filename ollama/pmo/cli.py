...
def validate(ctx: click.Context, verbose: bool, fail_on_score: int) -> None:
    ...
    try:
        ...
        click.echo(
            "Overall Score: "
            + click.style(f"{score}%", fg=color, bold=True)
            + f" ({passed_count}/{total_count} checks passed)"
        )
        click.echo(f"Status: {status_icon} " + click.style(status_text, fg=color, bold=True))
        click.echo(f"{'='*60}\n")
...
def remediate(ctx: click.Context, auto_fix: bool, dry_run: bool) -> None:
    ...
    try:
        ...
        if not dry_run and auto_fix:
            ...
            click.echo(f"\n{'='*60}")
            click.echo(f"Applied {result['applied']} fixes at {result['timestamp']}")
            click.echo(f"{'='*60}\n")
            ...
            if result["applied"] > 0:
                ...
                click.echo(click.style("⏭️  No fixes needed - repository compliant", fg="yellow"))
        else:
...
@cli.command()
@click.option("--force", is_flag=True, help="Overwrite existing labels")
@click.pass_context
def setup_labels(ctx: click.Context, force: bool) -> None:
    ...
    try:
        ...
        if result["failed"] > 0:
            click.echo(click.style(f"❌ Failed: {result['failed']} labels", fg="red"))
        ...
        if result["created"] > 0 or result["updated"] > 0:
            ...
            click.echo(click.style("⏭️  All labels already configured", fg="yellow"))
        ...
        if ai_powered:
            ...
            pmo_data["git"] = {
                "git_repo": click.prompt(
                    "Git repository",
                    default=(
                        f"github.com/{ctx.obj['repo']}"
                        if ctx.obj["repo"]
                        else "github.com/owner/repo"
                    ),
                ),
...
@click.option("--batch", "-b", is_flag=True, help="Process all issues in batch mode")
@click.option("--apply-labels", is_flag=True, help="Automatically apply suggested labels")
@click.option("--assign-team", is_flag=True, help="Automatically assign to recommended team")
@click.option("--find-duplicates", is_flag=True, help="Find and report duplicate issues")
@click.option(
    "--min-similarity",
    type=float,
    default=0.7,
        ...
        if output_format == "json":
            ...
            for result in results:
                ...
                click.echo(
                    "Type: "
                    + click.style(
                        result["issue_type"].upper(),
                        fg=type_color.get(result["issue_type"], "white"),
                        bold=True,
                    )
                )
                ...
                click.echo("Urgency: " + click.style(f"{urgency}/100", fg=urgency_color, bold=True))
                ...
                click.echo("Confidence: " + click.style(f"{confidence:.0%}", fg=conf_color))
