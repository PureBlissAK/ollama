...
class RemediationFix:
    ...
    fix_function: Any
    rollback_function: Any | None = None
...
class RemediationEngine:
    ...
    def remediate_advanced(
        ...
        all_fixes = [f for f in all_fixes if severity_order.get(f.severity, 0) >= min_severity]

        # Sort by severity (critical first)
        all_fixes.sort(key=lambda f: severity_order.get(f.severity, 0), reverse=True)
    ...
    def _rollback_file_change(
        self, fix: RemediationFix | None, rollback_data: dict[str, Any]
    ) -> bool:
        ...
        for result in reversed(self.results):
            if result.fix_id == fix_id and result.rollback_available:
                ...
                if fix and fix.rollback_function:
                    try:
                        res = fix.rollback_function(fix, result.rollback_data)
                        return bool(res)
                    except Exception:
                        return False
